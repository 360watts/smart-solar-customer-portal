...
export function HeroSection() {
  ...
  return (
    <motion.section
      ref={sectionRef}
      id="hero-section"
      className="relative mt-13 sm:mt-15 lg:mt-0 h-[30vh] sm:h-[40vh] md:h-[85vh] lg:h-screen overflow-hidden scroll-mt-20 bg-[#0c1e14]"
    ...
    >
      ...
      <div className="absolute inset-0 bg-linear-to-b from-transparent via-[rgba(30,30,30,0.4)] to-[rgba(30,30,30,0.8)]" />
            ...
            >
              ...
              >
                <Link
                  ...
                  className="inline-flex items-center justify-center gap-1 sm:gap-1.5 px-2.5 sm:px-4 md:px-6 lg:px-8 py-1.5 sm:py-2.5 md:py-3 lg:py-4 min-h-10 sm:min-h-11 text-[11px] sm:text-xs md:text-sm lg:text-base font-semibold bg-linear-to-r from-[#04713a] to-[#015c40] text-white rounded-md sm:rounded-lg md:rounded-xl hover:opacity-90 active:scale-95 transition-all"
                >
                  Get Free Consultation
                  <ArrowRight className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 lg:w-5 lg:h-5 shrink-0" />
                </Link>
              </motion.div>
            </motion.div>
          </AnimatePresence>
      ...
      <div className="absolute bottom-0 sm:bottom-4 md:bottom-6 lg:bottom-8 left-1/2 -translate-x-1/2 hidden lg:flex gap-1.5 sm:gap-2 z-20" role="tablist" aria-label="Hero slides">
        {heroSlides.map((_, index) => (
          <button
            ...
            className="min-w-9 min-h-9 sm:min-w-11 sm:min-h-11 flex items-center justify-center transition-all duration-300 hover:scale-110 active:scale-95"
          >
